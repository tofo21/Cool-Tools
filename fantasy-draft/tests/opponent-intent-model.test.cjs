...
assert.equal(packageData.metadata.market.snapshotId, "espn_2026_frozen_20260901T003012Z_3379127ab1c0");
assert.equal(packageData.metadata.market.captureTimestampUtc, "2026-09-01T00:30:12Z");
assert.equal(packageData.metadata.market.publicationCommit, "49951ca1d45b92a906f84366a02d40c8c2e07e12");
assert.equal(packageData.metadata.market.snapshotSha256, "e333dfbc3196351ea1b04f6fa8a5525db5903067f38318c8d2a725d6f75bc2a2");
assert.equal(packageData.metadata.market.schemaVersion, "espn-market-2026-v1.1");
assert.equal(packageData.metadata.market.status, "frozen");
assert.equal(packageData.metadata.market.coverage.sourceRows, 500);
assert.deepEqual(JSON.parse(JSON.stringify(packageData.metadata.market.coverage.draftCommandIdentities)), { mapped: 199, total: 200 });
assert.deepEqual(JSON.parse(JSON.stringify(packageData.metadata.market.coverage.mappedWithEspnDefaultRank)), { count: 199, total: 199 });
assert.deepEqual(JSON.parse(JSON.stringify(packageData.metadata.market.coverage.mappedWithContinuousEspnAdp)), { count: 199, total: 199 });
assert.deepEqual(JSON.parse(JSON.stringify(packageData.metadata.market.coverage.keepersRepresented)), { count: 10, total: 10 });
assert.equal(packageData.metadata.market.coverage.unresolvedEspnTop160, 0);
assert.equal(packageData.metadata.market.coverage.duplicateInternalPlayerIds, 0);
assert.equal(packageData.metadata.market.coverage.duplicateEspnPlayerIds, 0);
assert.deepEqual(JSON.parse(JSON.stringify(packageData.metadata.market.coverage.draftCommandOnlyMisses)), [{ playerId: 190, reason: "outside ESPN payload", blocking: false }]);
assert.equal(packageData.metadata.market.fieldPolicy.ordinalAdpRankCreated, false);
assert.equal(packageData.metadata.market.fieldPolicy.rankAdpBlended, false);
assert.equal(packageData.playerMarket.filter((entry) => entry.marketCoverage === "matched-current-espn").length, 199);
assert.equal(packageData.playerMarket.filter((entry) => entry.marketCoverage === "matched-current-espn" && entry.espnDefaultRank !== null).length, 199);
assert.equal(packageData.playerMarket.filter((entry) => entry.marketCoverage === "matched-current-espn" && entry.espnAdp !== null).length, 199);
assert.deepEqual(Array.from(packageData.playerMarket.filter((entry) => entry.marketCoverage !== "matched-current-espn"), (entry) => entry.playerId), [190]);
assert.ok(packageData.playerMarket.every((entry) => !Object.hasOwn(entry, "espnAdpRank")), "runtime package must not invent ordinal ESPN ADP rank");
